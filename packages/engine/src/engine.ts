...
import type {
  EnabledFeature,
  EnabledFeatureDeauthType,
} from "@airscan/types/EnabledFeature";
...
const deepMerge = <T extends object>(target: T, source: Partial<T>): T => {
  ...
  for (const key of Object.keys(source)) {
    ...
    if (
      ...
      const newProbes = sourceValue.filter(
...
export const createEngineStore = () => {
  return create<EngineState>((set, get) => ({
    ...
    updateEnabledFeature: (feature) =>
      set((state) => ({
        enabledFeatures: state.enabledFeatures.map((currentFeature) =>
...
export interface Engine {
  ...
  addEnabledFeature: (
    feature: Omit<EnabledFeature, "id" | "isActive">,
  ) => EnabledFeature;
  removeEnabledFeature: (feature: EnabledFeature) => void;
...
export const createEngine = (transportCallbacks?: {
