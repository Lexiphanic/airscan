...
export default function getFeatureDescription(feature: EnabledFeature): string {
  if (feature.type === "deauth") {
    if (feature.options.accessPoint && feature.options.station) return `${feature.options.station} ↔ ${feature.options.accessPoint}`;
    if (feature.options.accessPoint) return `All clients on ${feature.options.accessPoint}`;
    if (feature.options.station) return `Client ${feature.options.station} (any AP)`;
