...
export default function DeauthFeatureList() {
  ...
  const deauthFeatures = enabledFeatures.filter(
    (enabledFeature) => enabledFeature.type === "deauth",
  );
  const removeEnabledFeature = useAppStore(
    (state) => state.removeEnabledFeature,
  );
  const resumeEnabledFeature = useAppStore(
    (state) => state.resumeEnabledFeature,
  );
  const pauseEnabledFeature = useAppStore((state) => state.pauseEnabledFeature);
  const manufacturers = useGroupedManufacturers();

  const handleStopManufacturer = (
  ...
  const groupedByManufacturer: Record<
    string,
    {
      rawName: string;
      type: "accessPoint" | "station";
      features: EnabledFeatureDeauthType[];
    }
  > = {};
  ...
  const individualDeauthFeatures = deauthFeatures.filter(
    (feature) =>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    title={
                      feature.isActive ? "Pause feature" : "Resume feature"
                    }
                  >
                    {feature.isActive ? (
                ...
                <Card key={key} className="p-3">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1">
                    {features.slice(0, 3).map((feature) => (
                      <Badge
                        key={feature.id}
                        color={feature.isActive ? "red" : "gray"}
                        className="text-[10px]"
                      >
                        {type === "accessPoint"
                          ? feature.options.accessPoint
