...
import { WebSocketApiSchemaAsJson, type WebSocketApi } from '@airscan/types/Api/WebSocket';
...
export class WebSocketClient implements ITransport {
  ...
  connect(): void {
    ...
    this.ws.onmessage = (event) => {
      ...
      const msg: WebSocketApi = data.success ? data.data : {
        type: "addLog",
        log: {
      ...
      switch (msg.type) {
        ...
        case 'pong':
          ...
          break;
        default:
          this.callbacks.addLog('Unexpected message type from server: ' + (msg as any).type, 'error');
      }
    };
