...
test("Smoke test with @airscan/backend-mock", async ({ page }) => {
  ...
  const settingsButton = page.locator(
    'button[aria-label="Transport settings"]',
  );
  await settingsButton.click();

  // Wait for modal to appear
  await expect(
    page.getByRole("heading", { name: "Transport Settings" }),
  ).toBeVisible();
  ...
  await expect(page.locator('h2:has-text("Access Points (")')).not.toHaveText(
    "Access Points (0)",
  );
  await expect(page.locator('h2:has-text("Access Points")')).toContainText(
    /Access Points \((\d+)\)/,
    { timeout: 15000 },
  );

  // Wait for at least one client to appear
  await expect(page.locator('h2:has-text("Clients (")')).not.toHaveText(
    "Clients (0)",
  );
  await expect(page.locator('h2:has-text("Clients")')).toContainText(
    /Clients \((\d+)\)/,
    { timeout: 15000 },
  );
});
